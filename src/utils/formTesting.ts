 ...
 */
export function testFormValidation(formSchema: any, testCases: Array<{
  description: string;
...
export function generateFieldBoundaryTests(
  ...
  if ((config.type === 'string' || config.type === 'email' || config.type === 'phone') && config.min !== undefined) {
  ...
  if ((config.type === 'string' || config.type === 'email' || config.type === 'phone') && config.max !== undefined) {
 ...
 */
export function logTestResults(testResults: Array<{
  description: string;
  passed: boolean;
  values: Record<string, any>;
